-- Inclusive subscription end dates (production-safe).
--
-- DATA SAFETY:
-- - No DELETE / DROP TABLE / truncate.
-- - Only UPDATE subscriptions.end_date (and updated_at).
-- - Idempotent: only rows still matching the OLD exclusive formula
--   (end_date::date - start_date::date = plans.duration_days) are adjusted.
-- - Already-inclusive rows (diff = duration_days - 1) and manually edited
--   ends are left untouched.
-- - Re-running this migration does not shorten dates again.
--
-- Formula change: exclusive end (start + duration) -> inclusive (start + duration - 1).
