-- Remove subscription_status column from members table
-- This column is redundant since subscription status should be determined
-- from the actual subscriptions table, not cached in the members table

-- First, update the user_profiles view to remove the subscription_status reference
...
SELECT 
    ...
    -- subscription_status removed - should be determined from subscriptions table
    t.id as trainer_id,
    t.specialization,
    t.experience_years,
    t.bio,
...
-- Now remove the subscription_status column from members table
ALTER TABLE members DROP COLUMN IF EXISTS subscription_status;

-- Add a comment to document why this column was removed
COMMENT ON TABLE members IS 'Member information table. Subscription status should be determined from the subscriptions table, not cached here.';
