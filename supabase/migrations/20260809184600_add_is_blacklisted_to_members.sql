-- Blacklist flag on members (orthogonal to status). Notes reuse member_notes.

ALTER TABLE members
ADD COLUMN IF NOT EXISTS is_blacklisted BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN members.is_blacklisted IS
  'When true, member is blacklisted. Reason/notes live in member_notes.';

CREATE INDEX IF NOT EXISTS idx_members_is_blacklisted
ON members(is_blacklisted)
