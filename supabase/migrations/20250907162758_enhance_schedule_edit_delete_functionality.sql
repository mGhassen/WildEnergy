...
-- Migration: Add schedule code field for unique schedule identifiers like SCH-0000

-- Add code column to schedules table
ALTER TABLE schedules 
ADD COLUMN IF NOT EXISTS code TEXT UNIQUE;

-- Create function to generate unique schedule codes
...
BEGIN
  ...
  SELECT COALESCE(MAX(CAST(SUBSTRING(code FROM 'SCH-(\d+)') AS INTEGER)), 0) + 1
  INTO next_number
  FROM schedules
  WHERE code ~ '^SCH-\d+$';
  
  -- Format as SCH-XXXX (4 digits with leading zeros)
  schedule_code := 'SCH-' || LPAD(next_number::TEXT, 4, '0');
  
  RETURN schedule_code;
END;
$$ LANGUAGE plpgsql;

-- Create trigger to automatically generate code when inserting new schedules
CREATE OR REPLACE FUNCTION set_schedule_code()
RETURNS TRIGGER AS $$
BEGIN
  -- Only set code if it's not already provided
  IF NEW.code IS NULL OR NEW.code = '' THEN
    NEW.code := generate_schedule_code();
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_set_schedule_code
  BEFORE INSERT ON schedules
  FOR EACH ROW
  EXECUTE FUNCTION set_schedule_code();

-- Add index for code field for better performance
CREATE INDEX IF NOT EXISTS idx_schedules_code ON schedules(code);

-- Add comment for documentation
COMMENT ON COLUMN schedules.code IS 'Unique schedule identifier code in format SCH-XXXX (e.g., SCH-0001, SCH-0002)';
COMMENT ON FUNCTION generate_schedule_code() IS 'Generates unique schedule codes in format SCH-XXXX with auto-incrementing numbers';
COMMENT ON FUNCTION set_schedule_code() IS 'Trigger function to automatically set schedule code on insert if not provided';
COMMENT ON TRIGGER trigger_set_schedule_code ON schedules IS 'Automatically generates unique schedule code when inserting new schedules';
