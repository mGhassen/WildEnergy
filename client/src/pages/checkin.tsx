...
export default function CheckinPage() {
  ...
  const fetchCheckinInfo = async (qrCodeValue: string) => {
    try {
      ...
      // Call the new QR code info endpoint
            ...
            <div className="space-y-6">
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                ...
                <div className="space-y-2 text-sm text-green-700">
                  <p><strong>Course:</strong> {checkinInfo.course?.name || checkinInfo.course?.className || 'Unknown'}</p>
                  {checkinInfo.course?.description && (
                    <p><strong>Description:</strong> {checkinInfo.course.description}</p>
                  )}
                  <p><strong>Class:</strong> {checkinInfo.course?.class?.name || checkinInfo.course?.className || 'Unknown'}</p>
                  <p><strong>Trainer:</strong> {checkinInfo.course?.trainer ? `${checkinInfo.course.trainer.firstName} ${checkinInfo.course.trainer.lastName}` : 'Unknown'}</p>
