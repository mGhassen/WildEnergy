...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: updatedAccount, error: fetchError } = await supabaseServer()
      .from('user_profiles')
      .select('*')
      .eq('account_id', accountId)
      .single();
    ...
    if (fetchError) {
      ...
      return NextResponse.json({ error: 'Failed to fetch updated account data' }, { status: 500 });
    }
    ...
    return NextResponse.json({
      ...
      account: {
        ...
        trainer_id: updatedAccount.trainer_id
      }
    });
  } catch (e) {
